...
export async function POST(request: Request) {
  ...
  if (!RECAPTCHA_SECRET_KEY || !GMAIL_USER || !GMAIL_APP_PASSWORD) {
    console.error("Employment API Error: Missing environment variables.");
  ...
  try {
    ...
    if (!validation.success) {
      console.warn(
        "Employment form validation failed:",
    ...
    if (recaptchaData.success && recaptchaData.score >= 0.5) {
      ...
      // --- Nodemailer logic to send emails ---
      ...
      await transporter.sendMail({
        ...
        html: `
          ...
          <p><strong>Company Name:</strong> ${formData.organization || "Not provided"}</p>
      ...
      await transporter.sendMail({
        ...
        to: formData.email,
        subject:
          "Thank you for your interest in hiring through Reception House!",
        html: `
          ...
          <p><strong>Company:</strong> ${formData.organization || "Not provided"}</p>
