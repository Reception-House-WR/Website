...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      if (!strapiRes.ok) {
        console.error("Strapi Save Error:", await strapiRes.text());
      }
    } catch (strapiError) {
    ...
    await transporter.sendMail({
      ...
      html: `
        <h2>New Landlord/Housing Lead</h2>
        <p><strong>Name:</strong> ${formData.name}</p>
        <p><strong>Email:</strong> ${formData.email}</p>
        <p><strong>Phone:</strong> ${formData.phone || "Not provided"}</p>
        <p><strong>Property Details:</strong> ${formData.organization || "Not provided"}</p>
    ...
    await transporter.sendMail({
      ...
      to: formData.email,
      subject: "Thank you for your interest in housing partnership!",
      html: `
        <h2>Thank you for reaching out!</h2>
        <p>Hi ${formData.name.split(" ")[0]},</p>
        <p>We appreciate your interest in partnering with Reception House to provide housing solutions for newcomers.</p>
        <p>Our housing team will review your property details and get back to you shortly.</p>
