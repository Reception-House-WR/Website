...
export const Footer = () => {
  ...
  const cols = useMemo<FooterColumn[]>(() => buildFooterColumns(NAV as NavItem[]), []);
      ...
      <div className="container mx-auto px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-12">
          ...
          <div className="space-y-6">
            <div>
              <h2 className="font-bold text-xl mb-4">Reception House Waterloo Region</h2>
              <address className="not-italic space-y-2 text-sm">
                <div className="flex items-start gap-2">
                  ...
                  <a
                    ...
                    className="hover:underline focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-secondary transition-colors"
            ...
            <div className="space-y-3">
              <h3 className="font-semibold text-base">Accessibility at Reception House</h3>
              <p className="text-sm leading-relaxed">
                We are committed to providing accessible services in accordance with the 
                Accessibility for Ontarians with Disabilities Act (AODA). For alternate 
                formats or accessibility inquiries, contact{" "}
                <a
                  href="mailto:info@receptionhouse.ca"
                  className="hover:underline focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-secondary transition-colors font-medium"
                >
                  info@receptionhouse.ca
                </a>.
              </p>
              ...
              {socialLinks.map((social) => {
                  ...
                  return ( 
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="hover:opacity-70 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-secondary transition-opacity"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
             {cols.map((col, idx) => (
              <nav key={col.title} aria-labelledby={`footer-col-${idx}`}>
                <h3 id={`footer-col-${idx}`} className="font-semibold text-base mb-3">
                  {col.title}
                </h3>
                <ul className="space-y-2 text-sm">
                  {col.links.map(link => (
                    <li key={link.href}>
                      <Link
                        href={link.href}
                        className="underline decoration-transparent hover:decoration-inherit focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:ring-offset-2 focus:ring-offset-yellow-300 rounded-sm"
                      >
                        {link.label}
                      </Link>
                    </li>
        ...
        <div className="border-t border-foreground/10 pt-6 text-center">
          ...
          <nav aria-label="Legal links" className="flex flex-wrap justify-center gap-3 text-xs">
            <Link
              href="/privacy"
              className="hover:underline focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-secondary transition-colors"
            >
